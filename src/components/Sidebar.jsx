...
function Sidebar({ name, width = "w-72" }) {
  ...
  return (
    <div
      className={`h-screen ${width} flex flex-col items-center gap-4 bg-bg-1 pt-4`}
    >
      <div className="h-[5%]">
        <Logo width="w-48"></Logo>
      </div>
      <div className="h-[92%] flex flex-col justify-between w-full">
        <div className="w-full flex items-start justify-start flex-wrap gap-8 max-h-[70%] p-3 px-8 pt-6 overflow-scroll overflow-x-hidden roomMates">
          <RoomMate name={name} />
        </div>
